...
export function GamePage() {
  ...
  // Net chip change:
  //   Win:  winner gets stake - rake (rake taken from winnings, not added to loser's loss)
      ...
      <header className="flex-shrink-0 relative z-10 glass-panel border-b border-white/5 px-4 py-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
              ...
              style={{
                background: 'linear-gradient(180deg, #2A2A40 0%, #1A1C23 100%)',
                border: `1px solid ${!voiceActive ? 'rgba(255,255,255,0.1)' : muted ? 'rgba(255,51,102,0.5)' : voiceConnected ? 'rgba(0,255,157,0.5)' : 'rgba(255,255,255,0.1)'}`,
              }}
            >
              <span className="text-sm">{!voiceActive ? '🎙' : muted ? '🔇' : '🎙'}</span>
              {voiceActive && (
                <span
